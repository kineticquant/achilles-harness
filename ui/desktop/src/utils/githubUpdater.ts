...
export class GitHubUpdater {
  private readonly owner = process.env.GITHUB_OWNER || 'kineticquant';
  private readonly repo = process.env.GITHUB_REPO || 'achilles-harness';
  private readonly bundleName = process.env.GOOSE_BUNDLE_NAME || 'Achilles';
    ...
    try {
      ...
      const response = await fetch(this.apiUrl, {
        headers: {
          ...
          'User-Agent': `Achilles-Desktop/${app.getVersion()}`,
        },
      ...
      if (platform === 'darwin') {
        assetName = arch === 'arm64' ? 'Achilles-arm64.dmg' : 'Achilles-intel.dmg';
      } else if (platform === 'win32') {
        assetName = 'AchillesSetup.exe';
      } else {
        assetName = `${this.bundleName}-linux-${arch}.deb`;
      }
